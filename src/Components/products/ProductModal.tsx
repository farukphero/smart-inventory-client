...
import { X, Plus, Package, DollarSign, Layers, AlignLeft, AlertTriangle, TrendingUp, Tag, CheckCircle, Loader2 } from 'lucide-react'
import { useCreateProductMutation, useGetSingleProductQuery, useUpdateProductMutation } from "@/src/redux/api/Products/productsApi"
...
interface ProductModalProps {
	isOpen: boolean
	onClose: () => void
	id?: string   // if provided, we're editing an existing product
}
...
export function ProductModal({ isOpen, onClose, id }: ProductModalProps) {
	...
	const { data: singleProductData, isLoading: isSingleProductLoading } = useGetSingleProductQuery(id!, {
		skip: !id
	})
	...
	const validateForm = () => {
		...
		if (!newProduct.productName.trim()) newErrors.productName = 'Product name is required'
	...
	const handleSaveProduct = async () => {
		...
		const productData = {
			...
			minStockThreshold: newProduct.minStockThreshold ? parseInt(newProduct.minStockThreshold) : 5,
		...
		try {
			if (isEditMode) {
				const res = await updateProduct({ id: id!, ...productData }).unwrap()
				if (res.success) {
			...
			handleClose() // Resets the form and closes the modal
		} catch (error: any) {
	...
	// --- Accessibility and UI handlers (unchanged) ---
	useEffect(() => {
	...
	if (isEditMode && isSingleProductLoading) {
		return (
			<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
				<div className="bg-white rounded-2xl shadow-xl p-6 flex items-center gap-3 animate-pulse">
	...
	const modalTitle = isEditMode ? 'Edit Product' : 'Create New Product'
	...
	return (
		...
		>
			...
			>
				...
				<div className="relative bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 px-6 py-5 flex-shrink-0">
					...
					<div className="relative z-10">
						...
						<p className="text-white/80 text-sm mt-1">{modalDescription}</p>
					</div>
					<button
						onClick={handleClose}
						className="absolute top-5 right-5 text-white/80 hover:text-white transition-all hover:scale-110 duration-200 bg-white/10 rounded-lg p-1.5 backdrop-blur-sm z-10 cursor-pointer"
				...
				{/* Form Body (unchanged – same as your original code) */}
				<div
					ref={formRef}
					className="flex-1 overflow-y-auto p-6 space-y-6"
					style={{ scrollBehavior: 'smooth', scrollbarWidth: 'thin', scrollbarColor: '#cbd5e1 #f1f5f9' }}
				>
					<style jsx>{`
          ...
          `}</style>
					...
					{/* Product Name */}
					<div data-field="productName" className="group">
						...
						<div className="relative">
							...
							<input
								...
								className={`w-full pl-9 pr-4 py-2.5 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all duration-200 ${errors.productName
						...
						<div data-field="price">
							...
							<div className="relative">
								...
								<input
									...
									className={`w-full pl-9 pr-4 py-2.5 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all duration-200 ${errors.price
										? 'border-red-500 focus:border-red-500 bg-red-50/50'
										: 'border-gray-200 focus:border-indigo-500 hover:border-gray-300'
										}`}
								/>
						...
						<div data-field="stock">
							...
							<div className="relative">
								...
								<input
									...
									className={`w-full pl-9 pr-4 py-2.5 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all duration-200 ${errors.stock
										? 'border-red-500 focus:border-red-500 bg-red-50/50'
										: 'border-gray-200 focus:border-indigo-500 hover:border-gray-300'
										}`}
								/>
						...
						<div data-field="minStock">
							...
							<div className="relative">
								...
								<input
									...
									className={`w-full pl-9 pr-4 py-2.5 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all duration-200 ${errors.minStockThreshold
										? 'border-red-500 focus:border-red-500 bg-red-50/50'
										: 'border-gray-200 focus:border-indigo-500 hover:border-gray-300'
										}`}
								/>
							</div>
							<p className="text-xs mt-1.5 text-red-400">⚠️ Alert when stock falls below this value</p>
					...
					<div data-field="categoryId">
						...
						<div className="relative">
							...
							<select
								...
								className={`w-full pl-9 pr-4 py-2.5 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all duration-200 ${errors.categoryId
									? 'border-red-500 focus:border-red-500 bg-red-50/50'
									: 'border-gray-200 focus:border-indigo-500 hover:border-gray-300'
						...
						{isCategoriesError && (
							<p className="text-xs text-red-500 mt-1.5">Failed to load categories. Please refresh the page.</p>
						)}
						{errors.categoryId && !isCategoriesLoading && (
							<p className="text-xs text-red-500 mt-1.5">{errors.categoryId}</p>
						)}
					</div>
					...
					{newProduct.stockQuantity &&
						...
						parseInt(newProduct.stockQuantity) <= parseInt(newProduct.minStockThreshold) && (
							<div className="bg-gradient-to-r from-yellow-50 to-orange-50 border-l-4 border-yellow-500 rounded-xl p-4 flex items-start gap-3 shadow-sm">
								...
								<div className="flex-1">
									...
									<p className="text-yellow-700 text-sm mt-0.5">
										Current stock ({newProduct.stockQuantity} units) is at or below minimum stock level (
										{newProduct.minStockThreshold} units)
									</p>
				...
				<div className="sticky bottom-0 bg-white/95 backdrop-blur-sm px-6 py-4 border-t border-gray-100 flex-shrink-0">
					<div className="flex gap-3">
						<button
							...
							className="flex-1 px-4 py-2.5 border-2 border-gray-200 rounded-xl text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 font-medium disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
						>
							Cancel
						</button>
						<button
							...
							className="flex-1 px-4 py-2.5 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 text-white rounded-xl hover:shadow-lg hover:scale-[1.02] transition-all duration-200 font-medium flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 cursor-pointer"
						>
							{submitIcon}
							{submitButtonText}
						</button>
